...
import { readReferenceDiscoveryPlan } from "./discovery-store.mjs";
...
function parseArguments(argv) {
  if (!Array.isArray(argv) || argv.length !== 3 ||
      argv.some((value) => typeof value !== "string" || value.length === 0 ||
        value.includes("\0"))) {
    throw new TypeError("reference discovery owner requires a collection, session, and plan binding");
  }
  const [collectionDir, ordinalText, expectedPlanSha256] = argv;
  ...
  return { collectionDir, sessionOrdinal, expectedPlanSha256 };
}
  ...
  try {
    ...
    const execution = await runSession({
      ...
      validateBundle: (bundle) =>
        assertReferenceDiscoveryChildBundle(plan, session.ordinal, bundle),
